...
namespace JMC.Extension.Server.Handlers
{
    ...
    {
        private readonly ILogger<JMCDefinitionHandler> _logger;

        public JMCDefinitionHandler(ILogger<JMCDefinitionHandler> logger) => _logger = logger;

        public override async Task<LocationOrLocationLinks> Handle(DefinitionParams request, CancellationToken cancellationToken)
        {
            var file = ExtensionData.Workspaces.GetJMCFile(request.TextDocument.Uri);
            if (file == null) return new LocationOrLocationLinks(Array.Empty<LocationOrLocationLink>());
